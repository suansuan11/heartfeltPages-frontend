...
const fullCoverUrl = computed(() => {
  return `http://localhost:8080${articleModel.value.coverImg}?t=${Date.now()}`;
});
...
const updateArticle = async (clickState) => {
  ...
  console.log('Article data before update:', articleModel.value);

  try {
    await articleUpdateService(articleModel.value);
    ElMessage.success("更新成功");
    visibleDrawer.value = false;
    await articleList(); // Refresh the article list after update
  } catch (error) {
  ...
  <el-card class="page-container">
    <template #header>
      <div class="header">
        <span>日记管理</span>
        <div class="extra">
          <el-button type="primary" @click="showAddDialog('添加日记')">添加日记</el-button>
        </div>
      </div>
    </template>
    <!-- 搜索表单 -->
    ...
    <el-table :data="articles" style="width: 100%">
      ...
      <el-table-column label="操作" width="100">
        <template #default="{ row }">
          <el-button :icon="Edit" circle plain type="primary" @click="showEditDialog(row,'编辑日记')"></el-button>
  ...
  <el-drawer v-model="visibleDrawer" :title="drawerTitle" direction="rtl" size="50%">
    ...
    <el-form :model="articleModel" label-width="100px">
      ...
      <el-form-item label="日记封面">
        ...
        <el-upload class="cover-uploader" :auto-upload="true" :show-file-list="false"
                   action="/api/cover"
                   name="file"
                   :headers="{'Authorization':tokenStore.token}"
                   :on-success="uploadSuccess"
        >
          <el-image v-if="articleModel.coverImg" :src="fullCoverUrl" class="avatar"/>
      ...
      <el-form-item>
        <el-button type="primary" @click="drawerTitle==='添加日记'?addArticle('已发布'):updateArticle('已发布')">发布
        </el-button>
        <el-button type="info" @click="drawerTitle==='添加日记'?addArticle('草稿'):updateArticle('草稿')">草稿
        </el-button>
      </el-form-item>
    </el-form>
  </el-drawer>
